...
public class VideoController {
    ...
    @PostMapping
    public ResponseEntity saveVideo(@RequestParam(value = "videoThumbnail", required = false) MultipartFile picFile,
                                    @RequestParam(value = "videoFile", required = false) MultipartFile videoFile
    ) {
        try {
            Boolean bool = videoService.saveVideo(picFile, videoFile);
            if (!bool) {
                return null;
            }
    ...
    public ResponseEntity<Long> likeComment(@PathVariable("id") String videoId) {
        return movementsController.likeComment(videoId);
    }
